...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    if (_.isFunction(options.proxy)) {
    	// we need to do something with current proxy
     	options.proxy = ropts.proxy = options.proxy(ropts.uri)
    }

    var requestArgs = [
    	'uri', 'url', 'qs', 'method', 'headers', 'body', 'form', 'json',
    	'multipart', 'followRedirect', 'followAllRedirects', 'maxRedirects',
    	'encoding', 'pool', 'timeout', 'proxy', 'auth', 'oauth',
      'strictSSL', 'jar','aws'
    ];
    var requestOptions = _.pick.apply(this,[ropts].concat(requestArgs))

    if (options.debug) {
    	console.log('options for http request :\n' + JSON.stringify(options))
    }

    var req = request(requestOptions, function(error,response) {
        if (error) {
